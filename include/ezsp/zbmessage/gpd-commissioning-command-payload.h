...
public:
	 ...
	 */
	bool isMACsequenceNumberCapability() const {
		return (options & (1<<COM_OPTION_MAC_SEQ_CAPABILITY_BIT));
	}
	bool isRxOnCapability() const {
		return (options & (1<<COM_OPTION_RX_ON_CAPABILITY_BIT));
	}
	bool isApplicationInformationPresent() const {
		return (options & (1<<COM_OPTION_APPLICATION_INFORMATION_BIT));
	}
	bool isPANIdRequest() const {
		return (options & (1<<COM_OPTION_PAN_ID_REQUEST_BIT));
	}
	bool isGPSecurityKeyRequest() const {
		return (options & (1<<COM_OPTION_GP_SECURITY_KEY_REQUEST_BIT));
	}
	bool isFixedLocation() const {
		return (options & (1<<COM_OPTION_FIXED_LOCATION_BIT));
	}
	bool isExtendedOptionsFieldPresent() const {
		return (options & (1<<COM_OPTION_EXTENDED_OPTION_FIELD_BIT));
	}
	 ...
	 */
	uint8_t getExtendedOption() const {
	 ...
	 */
	uint32_t getOutFrameCounter() const {
