...
void CAes::set_key( const uint8_t key[AES_KEY_SIZE] )
{
    uint8_t cc, rc, hi;
...
bool CAes::cbc_encrypt(const unsigned char *in, unsigned char *out, unsigned long size, unsigned char iv[IAes::AES_BLOCK_SIZE])
{
    if (size % 16 != 0)
        return false;
