...
public:
	 ...
	 * @brief getters
	 */
	CEmberGpSinkTableOption getOption() const {
		return options;
	}
	CEmberGpAddressStruct getGpdAddr() const {
		return gpd;
	}
	EmberNodeId getAssignedAlias() const {
		return assigned_alias;
	}
	EmberGpSecurityFrameCounter getSecurityFrameCounter() const {
		return gpdSecurity_frame_counter;
	}
	EmberKeyData getGpdKey() const {
		return gpd_key;
	}
	uint8_t getGroupcastRadius() const {
		return groupcast_radius;
	}
	uint8_t getSecurityLevel() const {
		return security_options&0x03;
	}
	uint8_t getSecurityKeyType() const {
		return (static_cast<uint8_t>(security_options>>2)&0x07);
	}
	bool isActive() const {
		return status==0x01;
	}
	 ...
	 * @brief status setter
	 */
	void setEntryActive(bool i_active) {
		this->status=(i_active?0x01:0xFF);
	}

	/**
	 * @brief options setter
	 */
	void setOptions(CEmberGpSinkTableOption i_options) {
		options=i_options;
	}
	 ...
	 * @brief gpd setter
	 */
	void setGpdAddress(CEmberGpAddressStruct i_gpd_address) {
		gpd=i_gpd_address;
	}
	 ...
	 */
	void setDeviceId(uint8_t i_device_id) {
	 ...
	 */
	void setAlias(uint16_t i_alias) {
	 ...
	 */
	void setSecurityOption(uint8_t i_security_option) {
	 ...
	 */
	void setFrameCounter(EmberGpSecurityFrameCounter i_frm_counter) {
	 ...
	 */
	void setKey(EmberKeyData i_gpd_key) {
		gpd_key=i_gpd_key;
	}
